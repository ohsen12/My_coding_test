...
### 다른 사람 풀이
- 이런 미친 획기적인 방법이...
- split('구분자') 함수는 구분자를 기준으로 문자열을 분할하고 그 과정에서 구분자는 제거한다.
- 슬라이싱은 [start:stop:step] 이런 구조로 구성된다. 각각 시작인덱스(포함),종료인데스(미포함),간격(몇 개 씩 건너뛰며 선택할 지)을 뜻한다. [::2]는 문자열의 처음부터 끝까지 두칸마다 한 번 씩 요소를 선택하겠다는 것이다.(즉 짝수 인덱스의 문자만을 추출함)
- or 연산자는 첫 번째 피연산자가 False로 평가되면 두 번째 피연산자를 반환한다. 빈 문자열("")은 False로 평가되므로, or "EMPTY"를 사용하여 빈 문자열이 반환되는 경우 대신 "EMPTY" 문자열이 반환되도록 한다.
  ```python
  def solution(code):
