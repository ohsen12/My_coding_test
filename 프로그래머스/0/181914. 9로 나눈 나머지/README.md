...
### map 함수 
- 파이썬의 map 함수는 함수와 이터러블(문자열,리스트,튜플 등)을 인수로 받아, 이터러블의 각 요소에 주어진 함수를 적용한 결과를 반환한다. **map 함수는 결과를 map 객체로 반환**하며, 필요에 따라 이를 리스트나 다른 형태로 변환할 수 있다.
- map(함수,반복가능한객체)
